let randomNaumber=parseInt(Math.random() * 100+1);
...
if(palyGame){
  submit.addEventListener('click',function(e){
   ...
   console.log(guess)
   volidedateGuess(guess);
  })
}
function volidedateGuess(guess){
if(isNaN(guess)){
  ...
  if(numGuess===11){
    displayGuess(guess)
    displayMessage(`Game Over. Random number was ${randomNaumber}`)
    endGame()
...
if(guess===randomNaumber){
  displayMessage('You guessed it right');
  endGame();

}else if(guess<randomNaumber){
...
newGameButton.addEventListener('click',function(e){
  randomNaumber=parseInt(Math.random() * 100+1);
  prevGuess=[]
  numGuess=1;
